...
Let's see our second example of a category with families---the \textit{set-theoretic model} or \textit{set model}. This could be considered the ``classical'' model of type theory. The definition of the set model will be informal, and we'll leave it to the reader to formalise it in the style of Definition \ref{defn:cwf}.
...
\bibitem{coq} Thierry Coquand. \textit{What is a model of type theory?} Notes from the Special Year on Univalent Foundations of Mathematics, Institute for Advanced Study, Princeton, 2012. \url{https://ncatlab.org/ufias2012/files/cwf1.pdf}
...
{\color{red} Interpret projections, validate computation rule}
...
Given $s \in \Tm(\Gamma,A)$, we interpret $\refl_s \in {\color{red} \Ty\big((\Gamma,\, x:A,\, y:A\pi), (s =_A s) \big) ??}$ as $\int{\refl_s}(\tup{a}) = *$.
